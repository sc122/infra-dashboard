        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="text-xs text-muted-foreground">
              {repos[0] ? timeAgo(repos[0].pushed_at, locale) : ""}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-start">Language</TableHead>
                  <TableHead className="text-start">Visibility</TableHead>
                  <TableHead className="text-start">CI/CD</TableHead>
                  <TableHead className="text-start">{t("lastPush")}</TableHead>
              ...
              <TableBody>
                {repos.map((repo) => (
                  <TableRow key={repo.id}>
                    ...
                    <TableCell>
                      {repo.private ? (
                        <Badge variant="outline" className="text-xs"><Lock className="h-3 w-3 me-1" /> Private</Badge>
                      ) : (
                        <Badge variant="secondary" className="text-xs"><Globe className="h-3 w-3 me-1" /> Public</Badge>
                      )}
                    </TableCell>
                    <TableCell>
...
function timeAgo(date: string, locale = "en"): string {
  const now = Date.now();
